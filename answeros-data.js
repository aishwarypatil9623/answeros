...
(function () {
  ...
  function first(row, keys) { for (const key of keys) if (row && row[key] !== undefined && row[key] !== null && row[key] !== '') return row[key]; return ''; }
  ...
  function normalizeRow(row, index = 0) {
    ...
    const normalized = {
      schemaVersion: VERSION, id: String(first(row, ALIASES.id) || index + 1), rowIndex: index,
      date, paper: textValue(first(row, ALIASES.paper)), subject: textValue(first(row, ALIASES.subject)),
      theme: textValue(first(row, ALIASES.theme)), subtopic: textValue(first(row, ALIASES.subtopic)),
      question: textValue(first(row, ALIASES.question)), marks: marks === null ? 0 : marks, maxMarks,
      max: maxMarks, score10: maxMarks > 0 && marks !== null ? +(marks / maxMarks * 10).toFixed(2) : null,
      score: marks === null ? 0 : marks, directive: textValue(first(row, ALIASES.directive)),
      demandAddressed, demandPct: demandAddressed, demand: demandAddressed,
      status: textValue(first(row, ALIASES.status)) || 'PROCESSED', gapCategory: textValue(first(row, ALIASES.gapCategory)),
      feedback: first(row, ALIASES.feedback), learning: textValue(first(row, ALIASES.learning)), pdfDate: textValue(first(row, ALIASES.pdfDate)),
      bestIntro: textValue(first(row, ALIASES.bestIntro)), idealSubheadings: listValue(first(row, ALIASES.idealSubheadings)),
      mustHavePoints: listValue(first(row, ALIASES.mustHavePoints)), valueAdditions: listValue(first(row, ALIASES.valueAdditions)),
      keywords: listValue(first(row, ALIASES.keywords)), examples: listValue(first(row, ALIASES.examples)),
      bestConclusion: textValue(first(row, ALIASES.bestConclusion)), improvements: listValue(first(row, ALIASES.improvements)),
      topperEdge: textValue(first(row, ALIASES.topperEdge)), demandBreakdown: listValue(first(row, ALIASES.demandBreakdown)), raw: row
    };
  ...
  function parseCSV(text) {
    const rows = [], row = []; let current = row, field = '', quoted = false;
    ...
    current.push(field); if (current.some(Boolean)) rows.push(current);
  ...
  let syncing = false; const subscribers = new Set();
  function getStore() { return { ...store, answers: store.answers || normalizeRows(store.rows || []) }; }
  function publish() { const payload = getStore(); window.dispatchEvent(new CustomEvent('answeros:data-updated', { detail: payload })); subscribers.forEach(fn => { try { fn(payload); } catch (_) {} }); }
  function getAnswers(options = {}) { const answers = store.answers || normalizeRows(store.rows || []); return options.includeInvalid ? answers : answers.filter(a => a.valid); }
  ...
  function setRows(rows, meta = {}) {
    ...
    save(STORE_KEY, store); publish(); return getStore();
  }
  function getConfig() { return { ...config }; }
  function setConfig(patch = {}) { config = Object.assign({}, config, patch); save(CONFIG_KEY, config); window.dispatchEvent(new CustomEvent('answeros:config-updated', { detail: getConfig() })); maybeAutoSync(); return getConfig(); }

  function metrics() {
    const answers = getAnswers(), score = a => a.score10 ?? 0, total = answers.length, avg = total ? answers.reduce((s, a) => s + score(a), 0) / total : 0;
    const papers = {}, directives = {}, gaps = {};
    answers.forEach(a => {
      const p = a.paper || 'Unknown'; if (!papers[p]) papers[p] = { count: 0, total: 0 }; papers[p].count++; papers[p].total += score(a);
      if (a.directive) { if (!directives[a.directive]) directives[a.directive] = { count: 0, total: 0 }; directives[a.directive].count++; directives[a.directive].total += score(a); }
      if (a.gapCategory) gaps[a.gapCategory] = (gaps[a.gapCategory] || 0) + 1;
    });
    const dated = [...new Set(answers.map(a => a.date).filter(Boolean))].sort(); let currentStreak = 0, longestStreak = 0, run = 0;
    for (let i = 0; i < dated.length; i++) { const prev = i ? new Date(dated[i - 1]) : null, cur = new Date(dated[i]); if (!prev || (cur - prev) / 86400000 !== 1) run = 1; else run++; longestStreak = Math.max(longestStreak, run); }
    if (dated.length) { currentStreak = 1; for (let i = dated.length - 1; i > 0; i--) { if ((new Date(dated[i]) - new Date(dated[i - 1])) / 86400000 === 1) currentStreak++; else break; } }
    const demandRows = answers.filter(a => Number.isFinite(a.demandAddressed));
    return {
      schemaVersion: VERSION, total, averageScore10: +avg.toFixed(2), bestScore10: total ? Math.max(...answers.map(score)) : 0,
      worstScore10: total ? Math.min(...answers.map(score)) : 0, currentStreak, longestStreak,
      ...
      directives: Object.fromEntries(Object.entries(directives).map(([k, v]) => [k, { count: v.count, averageScore10: +(v.total / v.count).toFixed(2) }])), gaps, dates: dated
    };
  }

  function buildSyncUrl() { if (!config.syncUrl) throw new Error('Google Sheet sync URL is not configured.'); const url = new URL(config.syncUrl); if (config.syncToken) url.searchParams.set('token', config.syncToken); url.searchParams.set('_', Date.now()); return url.toString(); }
  async function sync() {
    if (syncing) return { skipped: true, count: getRows().length, lastSync: store.lastSync }; syncing = true;
    try {
      const response = await fetch(buildSyncUrl(), { method: 'GET', mode: 'cors', cache: 'no-store' }); if (!response.ok) throw new Error('HTTP ' + response.status);
      const contentType = response.headers.get('content-type') || '', text = await response.text();
      const data = contentType.includes('json') || text.trim().startsWith('{') || text.trim().startsWith('[') ? JSON.parse(text) : { rows: parseCSV(text) };
  ...
  function maybeAutoSync() { if (!config.autoSyncEnabled || !config.syncUrl) return; const age = store.lastSync ? Date.now() - new Date(store.lastSync).getTime() : Infinity; const interval = Math.max(5, Number(config.syncIntervalMinutes) || 15) * 60000; if (age >= interval) sync().catch(() => {}); }

  window.AnswerOS = {
    VERSION, SCHEMA: { version: VERSION, fields: ALIASES }, normalizeRow, normalizeRows, validate, metrics,
  ...
  if ((!store.rows || !store.rows.length) && oldStore && Array.isArray(oldStore.rows) && oldStore.rows.length) {
    store.rows = oldStore.rows; store.answers = normalizeRows(oldStore.rows); store.lastSync = oldStore.lastSync || null; store.source = 'migrated-v1'; store.validation = validate(store.answers); save(STORE_KEY, store);
  } else if (Array.isArray(store.rows) && store.rows.length && (!store.answers || !store.answers.length)) {
    store.answers = normalizeRows(store.rows); store.validation = validate(store.answers); save(STORE_KEY, store);
  }
  window.addEventListener('storage', event => {
    if (event.key === STORE_KEY) { store = read(STORE_KEY, store); store.answers = normalizeRows(store.rows || []); publish(); }
    if (event.key === CONFIG_KEY) { config = Object.assign({}, DEFAULT_CONFIG, read(CONFIG_KEY, config)); maybeAutoSync(); }
  });
