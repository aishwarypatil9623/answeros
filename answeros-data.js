...
(function () {
  ...
  const DEFAULT = { syncUrl: '', syncToken: '', autoSyncEnabled: false, syncIntervalMinutes: 15 };
  ...
  function normaliseRows(rows) {
    return rows.map((row, index) => {
      ...
      return Object.assign({}, row, {
        ...
        subject: pick('subject', 'Subject', 'paper', 'Paper'),
  ...
  window.addEventListener('storage', (event) => {
    ...
    if (event.key === CONFIG_KEY) config = Object.assign({}, DEFAULT, read(CONFIG_KEY, config));
  });
})();
